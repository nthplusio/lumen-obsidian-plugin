/**
 * Search Sidebar View for Lumen.
 *
 * Registers a custom sidebar panel in Obsidian with a search input,
 * debounced queries to the MCP endpoint, clickable results list with
 * markdown-rendered snippets, and comprehensive error handling with retry.
 */
...
export const VIEW_TYPE_LUMEN_SEARCH = 'lumen-search-view';

/** Maximum retry attempts for transient failures */
const MAX_RETRIES = 2;
const RETRY_DELAY_MS = 1000;

export class LumenSearchView extends ItemView {
	...
	async onOpen(): Promise<void> {
		...
		if (!container) return;
		container.empty();
		container.addClass('lumen-search-container');
	...
	private formatRelativeDate(dateStr: string): string {
		...
		if (diffDays < 30) return `Modified ${Math.floor(diffDays / 7)} weeks ago`;
